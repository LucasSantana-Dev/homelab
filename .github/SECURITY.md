...
- **Public-Safe Tracking Policy**: Tracked docs/config examples must use pseudonymized identifiers
...
4. **Public Repository Hygiene**:
   - Run `./scripts/security/secret-gate.sh` before pushing
   - Run `./scripts/security/public-safety-gate.sh` before pushing
   - Keep production identifiers (domains, private hostnames, tailnet names, IPs) out of tracked docs/config
...
For credential exposure reports, include:

- Affected commit hash(es)
- Exposed secret class (token/webhook/password/key)
- Current rotation/revocation status
