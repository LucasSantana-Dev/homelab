...
class TestHomelabUpdateManager:
    ...
    def test_init(self):
        ...
        with patch('homelab_manager.updates.docker.from_env') as mock_docker:
            ...
            assert manager.homelab_dir == Path(__file__).parent.parent.parent.parent.parent
    ...
    def test_init_docker_not_available(self):
        ...
        with patch('homelab_manager.updates.docker.from_env', side_effect=Exception("Docker not available")):
    ...
    def test_check_image_updates_no_update(self):
        ...
        with patch('homelab_manager.updates.docker.from_env') as mock_docker:
            ...
            with patch('rich.console.Console.print') as mock_print:
                manager = HomelabUpdateManager()
                has_update, current_id, latest_id = manager.check_image_updates("test/image", "latest")

                assert has_update is False
                assert current_id == "abc123"
                assert latest_id == "abc123"
                mock_print.assert_called_with("📥 Pulling latest image for test/image...", style="blue")
    ...
    def test_check_image_updates_error(self):
        ...
        with patch('homelab_manager.updates.docker.from_env') as mock_docker:
            ...
            with patch('rich.console.Console.print') as mock_print:
                ...
                mock_print.assert_called_with("⚠️ Error checking test/image: Docker error", style="yellow")
    ...
    def test_update_service_success(self):
        ...
        with patch('homelab_manager.updates.docker.from_env') as mock_docker:
            ...
            with patch('homelab_manager.updates.subprocess.run') as mock_run, \
                ...
                mock_print.assert_any_call("✅ test-service updated successfully", style="green")
    ...
    def test_update_service_subprocess_error(self):
        ...
        with patch('homelab_manager.updates.docker.from_env') as mock_docker:
            ...
            with patch('homelab_manager.updates.subprocess.run', side_effect=subprocess.CalledProcessError(1, "docker-compose")), \
                 patch('homelab_manager.updates.os.chdir'), \
                 patch('homelab_manager.updates.console.print') as mock_print:
                ...
                assert result is False
                mock_print.assert_any_call("❌ Update failed: Command 'docker-compose' returned non-zero exit status 1.", style="red")
    ...
    def test_update_all_services_success(self):
        ...
        with patch('homelab_manager.updates.docker.from_env') as mock_docker:
            ...
            with patch('homelab_manager.updates.subprocess.run') as mock_run, \
                 patch('homelab_manager.updates.os.chdir'), \
                 patch('homelab_manager.updates.console.print') as mock_print, \
                 patch('homelab_manager.updates.HomelabAutomation') as mock_automation_class, \
                 patch('homelab_manager.updates.HomelabHealthMonitor') as mock_health_class:
    ...
    def test_update_all_services_backup_failure(self):
        ...
        with patch('homelab_manager.updates.docker.from_env') as mock_docker:
            ...
            with patch('homelab_manager.updates.console.print') as mock_print, \
                 patch('homelab_manager.updates.HomelabAutomation') as mock_automation_class:
                ...
                assert result is False
                mock_print.assert_any_call("❌ Backup failed, aborting update", style="red")
    ...
    def test_show_versions_error(self):
        ...
        with patch('homelab_manager.updates.docker.from_env') as mock_docker:
            ...
            with patch('rich.console.Console.print') as mock_print:
                ...
                mock_print.assert_any_call("⚠️ Error getting versions: Docker error", style="yellow")
    ...
    def test_main_function_check(self):
        ...
        with patch('homelab_manager.updates.HomelabUpdateManager') as mock_manager_class:
            mock_manager = Mock()
    ...
    def test_main_function_update_service_no_service(self):
        ...
        with patch('homelab_manager.updates.HomelabUpdateManager') as mock_manager_class:
            ...
            with patch('homelab_manager.updates.argparse.ArgumentParser') as mock_parser, \
                 patch('homelab_manager.updates.console.print') as mock_print, \
                ...
                mock_print.assert_called_with("❌ Please specify --service for update-service", style="red")
                mock_exit.assert_called_with(1)
